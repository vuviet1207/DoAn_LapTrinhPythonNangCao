...
class Calculator:
    ...
    def _msgBox(self):
        msg.showinfo('Thông tin Máy tính', 'Đây là máy tính đơn giản sử dụng tkinter\n Truong Viet Vu_2274802011045')

    def reset(self):
        # Đặt lại các tab về trạng thái mặc định
        self.tabControl.forget(self.tab1)
        self.tabControl.forget(self.tab2)
        self.tabControl.forget(self.tab3)
        self.tabControl.forget(self.tab4)
        self.tabControl.forget(self.tab5)
        self.tabControl.forget(self.tab6)

        # Khởi tạo lại các tab
        self.tab1 = Tong(self.tabControl)
        self.tabControl.add(self.tab1, text="Tổng")
        self.tabControl.pack(expand=1, fill="both")
