...
class SearchBooks:
    def __init__(self, tab, app):
        ...
        tk.Button(search_frame, text="Tìm kiếm", command=lambda: self.search_data(self.app.table_name.get())).grid(row=0, column=2, padx=5, pady=5)
    ...
    def search_data(self, table_name):
        try:
            search_query = sql.SQL("SELECT * FROM {} WHERE ten_sach ILIKE %s").format(sql.Identifier(table_name))
...
class AddBooks:
    def __init__(self, tab, app):
        ...
        tk.Button(insert_frame, text="Thêm sách", command=lambda: self.insert_data(self.app.table_name.get())).grid(row=4, columnspan=2, pady=10)
    ...
    def insert_data(self, table_name):
        try:
            ...
            add_query = sql.SQL("INSERT INTO {} (ten_sach, tac_gia, nxb, the_loai) VALUES (%s, %s, %s, %s)").format(sql.Identifier(table_name))
...
class UpdateBooks:
    def __init__(self, tab, app):
        ...
        tk.Button(update_frame, text="Cập nhật dữ liệu", command=lambda: self.update_data(self.app.table_name.get())).grid(row=5, columnspan=2, pady=10)
    ...
    def update_data(self, table_name):
        try:
            ...
            update_query = sql.SQL("UPDATE {} SET ten_sach=%s, tac_gia=%s, nxb=%s, the_loai=%s WHERE id=%s").format(sql.Identifier(table_name))
...
class DeleteBooks:
    def __init__(self, tab, app):
        ...
        tk.Button(delete_frame, text="Xóa sách", command=lambda: self.delete_data(self.app.table_name.get())).grid(row=1, columnspan=2, pady=10)
